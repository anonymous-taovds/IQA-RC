...
class Kadid_10kFolder(data.Dataset):
    ...
    def __init__(self, root, index, transform, patch_num):
        ...
        mos_all = [(9 * i - 5) / 4 for i in mos_all]
        refnames_all = np.array(refnames_all)

        for j in range(10125):
            mos_temp = []
            for k in range(1, 11):
                if (k == int(mos_all[j])):
                    mos_temp.append(k+1-mos_all[j])
                elif (k == int(mos_all[j])+1):
                    mos_temp.append(mos_all[j]-int(mos_all[j]))
                else:
                    mos_temp.append(0)
            mos_new.append(mos_temp)

        mos_new = np.array(mos_new)
        sample = []
        for i, item in enumerate(index):
            train_sel = (refname[index[i]] == refnames_all)
            train_sel = np.where(train_sel == True)
            ...
            for j, item in enumerate(train_sel):
                for aug in range(patch_num):
                    sample.append((os.path.join(root, 'images', imgname[item]), mos_new[item], mos_all[item], imgname[item]))

        # for i, item in enumerate(index):
        #     for aug in range(patch_num):
        #         sample.append((os.path.join(root, 'images', imgname[item]), mos_new[item]))
    ...
    def __getitem__(self, index):
        ...
        Returns:
            tuple: (sample, target) where target is class_index of the target class.
        """
        path, target, target1, name = self.samples[index]
        sample = pil_loader_1(path)
        sample = self.transform(sample)
        return sample, target, target1, name
...
class TID2013Folder(data.Dataset):
    ...
    def __init__(self, root, index, transform, patch_num):
        ...
        for line in fh:
            ...
            refnames_all.append(ref_temp[0][1:])
        mos_all = np.array(target).astype(np.float32)
        refnames_all = np.array(refnames_all)

        mos_all = [(i+1) for i in mos_all]
        for j in range(3000):
            mos_temp = []
            for k in range(1, 11):
                if (k == int(mos_all[j])):
        ...
        for i, item in enumerate(index):
            ...
            for j, item in enumerate(train_sel):
                for aug in range(patch_num):
                    sample.append((os.path.join(root, 'distorted_images', imgnames[item]), mos_new[item], mos_all[item], imgnames[item]))
        self.samples = sample
        self.transform = transform
