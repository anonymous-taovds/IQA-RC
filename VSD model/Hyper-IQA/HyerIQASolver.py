...
class HyperIQASolver(object):
    ...
    def __init__(self, config, path, train_idx, test_idx):
        ...
        # self.model_hyper_1 = models.HyperNet(16, 112, 1024, 112, 56, 28, 14, 7).cuda()
    ...
    def train(self):
        ...
        for t in range(self.epochs):
            ...
            # cunmoxing
            if (t + 1) > 5:
                save_dir = os.path.join(modelDir, 'model-%04d.pth' % (t + 1))
                state = {
                    'network': self.model_hyper.state_dict(),
                    # 'optimizer':NDnet_opt.state_dict(),
                    # 'ep': curep+1,
                    # 'total_it': total_it
                }
    ...
    def test(self):
        ...
        for img, label in self.test_data:
            ...
            pred1 = float(pred.item())

            # print('%d:' % (item) + str(name) + '|score: %.3f' % (pred1))
            # with open(os.path.join(root, 'test.txt'), 'a') as f:
            #     f.write('%d:' % (item) + str(name) + '|score: %.3f\n' % (pred1))
